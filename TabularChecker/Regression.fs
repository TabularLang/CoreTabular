...
let rec checkPredictor dim G es e =
        ...
        | Variable (c,_) -> 
          match Map.tryFind c G with
                   | Some (Arrays (Some) es u,sort) -> 
                     ...
                     else TypedPredictor(Variable(c,sort),u) // we annotate with sort for correct elaboration
                   | Some (t,_) -> failwithf "Variable %s has type %s, expecting array type with dimensions %s" (Pretty.PredictorToString e) (Pretty.columnTypeToStr t)  (String.concat "" (List.map (fun e -> "["+(Pretty.exprToStr e)+"]") es))
        ...
        | Interaction(p1,p2) ->
          match checkPredictor dim G es p1  ,checkPredictor dim G es p2 with
               | (TypedPredictor(e1,t1)) as pt1,(TypedPredictor(e2,t2) as pt2) when areTypesEquivalent G_Empty t1 dim  && areTypesEquivalent G_Empty t2 dim ->
                  TypedPredictor(Interaction(pt1,pt2),dim)
        ...
        | Path(ps,p) ->
          let rec checkPaths ps pts fs  =
               ...
               | [] ->
                 ...
                 | TypedPredictor(p,t) as pt  -> 
                   TypedPredictor(Path(List.rev pts,pt),t)
               | (pi::ps) ->
                 match checkPredictor dim G es pi with
                 | (TypedPredictor(pi,T_Upto(fi)) as pit) ->
                     checkPaths ps (pit::pts) (fi::fs) 
                 | (TypedPredictor(pi,ti) as pit) -> failwithf "Ill-typed path component %s; expecting predictor of 'mod(_)' type, found predictor of type %s " (Pretty.PredictorToString pi) (Pretty.columnTypeToStr ti)
          checkPaths ps [] []
        | TypedPredictor _ -> failwith "checkPredictor"


 //TODO rewrite this for vectorization
let synthTargetType dim G es e =
        //match dim with
       // | T_Real -> 
           match checkPredictor dim G es e with 
           | (TypedPredictor(e,t)) as et when areTypesEquivalent G_Empty t dim ->
               et
           | (TypedPredictor(e,t)) -> failwithf "expected predictor type %s, found predictor of type %s" (Pretty.columnTypeToStr dim)  (Pretty.columnTypeToStr t) 
      (*TBC
        | T_Array(T_Real,e1) ->
           match checkPredictor dim G e es  with 
           | (TypedPredictor(e,T_Real)) as et ->
              ((TypedPredictor(vecE y et,y)))
           | (TypedPredictor(e,t) as et) when areTypesEquivalent G_Empty y t ->
              et
           | (TypedPredictor(e,t)) -> failwithf "expected predictor of type real or %s, found predictor of type %s" (Pretty.columnTypeToStr y)  (Pretty.columnTypeToStr t) 
        | T_Array(T_Real,_) -> // allow implicit coercion from real to real [] -- use a let?
           try let et = checkPredictor G e  es  
               ((TypedExp(vecE y et,y)))
           with _ ->
               checkPredictor G e es
 *)            
let rec checkRegrn (G:Map<ColumnName,ColumnType* (Level*TableName) option>) es fs (r:Regression) (dim:TargetType) : (Regression*RecordType) =
    match r with 
    | Immed e -> 
        let et = synthTargetType dim G es e
        (Immed et,[])
    | Cond (r,p,_) ->
        let (TypedPredictor(e,t) as pt) = checkPredictor dim G es p
        let f = match t with 
                 | T_Upto(f) -> f 
    ...
    | Coeff(e,alpha,r) ->
        ...
        let et = synthTargetType dim G es e
        let (rt,Q) = checkRegrn G fs [] r (dim:TargetType)
        if List.exists (fun (c,_) -> c = alpha) Q then failwithf "duplicate parameter name %s in regression" alpha
        (Coeff(et,alpha,rt),arrays ((alpha,vecT dim R_Real)::Q) fs)
    | Noise(d,ps) ->
        let pts = List.map (fun p -> checkPredictor T_Real G fs p) ps
        let ets = List.mapi (fun (i:int) (TypedPredictor(p,t)) -> TypedExp(Syntax.Var (i.ToString()), t)) pts
        let t = Checker.synthDist W G_Empty (d,ets)
        match t with
        | T_Real -> 
          (Noise(d,pts),[])
        | _ -> failwith "Noise term %s expected to have type real but has type %s" (Pretty.RtoString r) (Pretty.columnTypeToStr t)
    | Res(v,r) ->
...
let check g r dim = checkRegrn (GammaOfEnv g) [] [] r dim 